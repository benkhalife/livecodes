...
(() => {
  ...
  var modulesService = {
    getModuleUrl: (moduleName, {
      ...
      const addExternalParam = (url2) => !external || !url2.includes("https://esm.sh") ? url2 : url2.includes("?") ? `${url2}&external=${external}` : `${url2}?external=${external}`;
  ...
  var getAppCDN = () => {
    ...
    try {
      ...
      return url2.searchParams.get("appCDN") || modulesService.cdnLists.npm[0];
    } catch {
      return modulesService.cdnLists.npm[0];
    }
  };
  ...
  var url = nunjucksBaseUrl + "nunjucks.min.js";
  var runtimeUrl = nunjucksBaseUrl + "nunjucks-slim.min.js";
  var nunjucks = {
    name: "nunjucks",
    title: "Nunjucks",
    parser: {
  ...
  // src/livecodes/languages/nunjucks/lang-nunjucks-compiler.ts
  self.createNunjucksCompiler = () => async (code, { config }) => {
    const options = getLanguageCustomSettings("nunjucks", config);
    self.nunjucks.configure(options);
    const data = config.customSettings.template?.data || {};
    if (config.customSettings.template?.prerender !== false) {
      const template = self.nunjucks.compile(code);
      return template.render(data);
    }
    const clientFn = self.nunjucks.precompileString(code, { name: "template" });
    return `<!-- ... compiling ... -->
