...
// src/livecodes/i18n/locale-paths.ts
var pathLoader = (baseUrl) => (lngs, nss) => {
  const lng = lngs[0];
  const ns = nss[0];
  if (lng === "ar" && ns === "language-info") {
    return baseUrl + "i18n-ar-language-info.json";
  }
  if (lng === "ar" && ns === "translation") {
    return baseUrl + "i18n-ar-translation.json";
  }
  if (lng === "de" && ns === "language-info") {
    return baseUrl + "i18n-de-language-info.json";
  }
  if (lng === "de" && ns === "translation") {
    return baseUrl + "i18n-de-translation.json";
  }
  if (lng === "en" && ns === "language-info") {
    return baseUrl + "i18n-en-language-info.json";
  }
  if (lng === "en" && ns === "translation") {
    return baseUrl + "i18n-en-translation.json";
  }
  if (lng === "es" && ns === "language-info") {
    return baseUrl + "i18n-es-language-info.json";
  }
  if (lng === "es" && ns === "translation") {
    return baseUrl + "i18n-es-translation.json";
  }
  if (lng === "fa" && ns === "language-info") {
    return baseUrl + "i18n-fa-language-info.json";
  }
  if (lng === "fa" && ns === "translation") {
    return baseUrl + "i18n-fa-translation.json";
  }
  if (lng === "fr" && ns === "language-info") {
    return baseUrl + "i18n-fr-language-info.json";
  }
  if (lng === "fr" && ns === "translation") {
    return baseUrl + "i18n-fr-translation.json";
  }
  if (lng === "hi" && ns === "language-info") {
    return baseUrl + "i18n-hi-language-info.json";
  }
  if (lng === "hi" && ns === "translation") {
    return baseUrl + "i18n-hi-translation.json";
  }
  if (lng === "it" && ns === "language-info") {
    return baseUrl + "i18n-it-language-info.json";
  }
  if (lng === "it" && ns === "translation") {
    return baseUrl + "i18n-it-translation.json";
  }
  if (lng === "ja" && ns === "language-info") {
    return baseUrl + "i18n-ja-language-info.json";
  }
  if (lng === "ja" && ns === "translation") {
    return baseUrl + "i18n-ja-translation.json";
  }
  if (lng === "pt" && ns === "language-info") {
    return baseUrl + "i18n-pt-language-info.json";
  }
  if (lng === "pt" && ns === "translation") {
    return baseUrl + "i18n-pt-translation.json";
  }
  if (lng === "ru" && ns === "language-info") {
    return baseUrl + "i18n-ru-language-info.json";
  }
  if (lng === "ru" && ns === "translation") {
    return baseUrl + "i18n-ru-translation.json";
  }
  if (lng === "tr" && ns === "language-info") {
    return baseUrl + "i18n-tr-language-info.json";
  }
  if (lng === "tr" && ns === "translation") {
    return baseUrl + "i18n-tr-translation.json";
  }
  if (lng === "ur" && ns === "language-info") {
    return baseUrl + "i18n-ur-language-info.json";
  }
  if (lng === "ur" && ns === "translation") {
    return baseUrl + "i18n-ur-translation.json";
  }
  if (lng === "zh-CN" && ns === "language-info") {
    return baseUrl + "i18n-zh-CN-language-info.json";
  }
  if (lng === "zh-CN" && ns === "translation") {
    return baseUrl + "i18n-zh-CN-translation.json";
  }
  return false;
};

// src/livecodes/i18n/utils.ts
var abstractifyHTML = (html) => {
  ...
  const replaceElement = (node) => {
    ...
    if (name === "body")
      return;
    const attributes = node.attributes.length === 0 ? void 0 : Array.from(node.attributes).reduce(
      (acc, attr) => {
...
var unabstractifyHTML = (html, elements, interpolation = {}) => {
  const parser = new DOMParser();
...
// src/livecodes/i18n/i18n.ts
var init2 = async (lng, baseUrl) => {
  await instance.use(esm_default).init({
    lng,
  ...
  return {
    translate,
    translateString,
    translateKey: instance.t.bind(instance),
    getLanguage: () => instance.language,
    getLanguageDirection: instance.dir.bind(instance),
    changeLanguage: instance.changeLanguage.bind(instance),
    loadNamespaces: instance.loadNamespaces.bind(instance)
  };
};
var translate = (container) => {
  if (!container)
    return;
  container.querySelectorAll("[data-i18n]").forEach((el) => {
    ...
    const translateProp = (prop, lookupKey) => {
      fullInterpolation.PROP = prop;
      if (prop.startsWith("data-")) {
        prop = prop.slice(5);
        el.dataset[prop] = instance.t(lookupKey, {
          defaultValue: el.dataset[prop],
          ...fullInterpolation
        });
      } else {
        const translation = instance.t(lookupKey, {
          defaultValue: el[prop],
          ...fullInterpolation
        });
...
var translateString = (key, value, ...args) => {
  if (!instance)
    return value;
  ...
  if (!interpolation || !isHTML) {
    ...
    const { elements } = abstractifyHTML(value);
