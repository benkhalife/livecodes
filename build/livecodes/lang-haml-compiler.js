...
(() => {
  ...
  // src/livecodes/languages/haml/lang-haml-compiler.ts
  self.createHamlCompiler = () => async (code, { config }) => {
    const options = {
      tolerateFaults: true,
    ...
    if (config.customSettings.template?.prerender !== false) {
      const fn = window.haml.compileHaml(options);
      return fn(data);
    }
    const clientFnSrc = window.haml.compileHaml({
      ...options,
      outputFormat: "string"
    });
    return `<!-- ... compiling ... -->
