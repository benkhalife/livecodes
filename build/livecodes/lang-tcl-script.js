...
(() => {
  ...
  // src/livecodes/languages/tcl/lang-tcl-script.ts
  var cdnBaselUrl = vendorsBaseUrl + "wacl/";
  ...
  livecodes.tcl.run = livecodes.tcl.run || ((input) => new Promise(async (resolve) => {
    ...
    const runCode = async (code2, input2, interp) => {
      const originalPrompt = window.prompt;
      window.prompt = () => input2;
      let output = null;
      let error = null;
      interp.stdout = (val) => {
        output = (output ?? "") + val + "\n";
