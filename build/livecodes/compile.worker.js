...
var doOnce = (fn) => {
  let dependency;
  return () => dependency ?? (dependency = fn());
};
...
).reduce((compilers2, language) => {
  if (language.compiler && !compilers2[language.name]) {
    if (typeof language.compiler === "string") {
      const compiler = languages2.find((lang) => lang.name === language.compiler)?.compiler;
      compilers2[language.name] = {
        ...compiler,
        url: getCompilerUrl(compiler.url, baseUrl2),
...
var stylis = {
  ...
  compiler: {
    url: stylisUrl,
    factory: () => async (code) => {
      const { compile: compile2, serialize, stringify: stringify2, middleware, prefixer } = window.stylis;
      return serialize(compile2(code), middleware([prefixer, stringify2]));
    }
  },
  extensions: ["stylis"],
  editor: "style",
...
var typescriptAtaUrl = vendorsBaseUrl + "typescript-ata/typescript-ata.js";
var typescriptVfsUrl = vendorsBaseUrl + "typescript-vfs/typescript-vfs.js";
var compilers;
var baseUrl;
var tsvfsMap;
var codemirrorWorker = { language: "tsx" };
var worker = self;
self.window = self;
self.deps = { languages, processors };
var loadLanguageCompiler = async (language, config, baseUrl2) => {
  if (!baseUrl2) {
    throw new Error("baseUrl is not set");
  }
  if (!compilers) {
    compilers = getAllCompilers([...languages, ...processors], config, baseUrl2);
  }
  ...
  if (languageCompiler.dependencies && languageCompiler.dependencies.length > 0) {
    for (const dependency of languageCompiler.dependencies) {
      await loadLanguageCompiler(dependency, config, baseUrl2);
    }
  }
    ...
    if (languageCompiler.aliasTo && typeof compilers[languageCompiler.aliasTo]?.fn === "function") {
      ...
      const load = async () => {
        try {
          ...
          languageCompiler.fn = await languageCompiler.factory(config, baseUrl2);
  ...
  const loadedMessage = { type: "loaded", payload: language };
  worker.postMessage(loadedMessage);
};
var compile = async (content, language, config, options) => {
...
var loadTypeScript = async () => {
  if (worker.ts)
    return;
  await loadLanguageCompiler("typescript", {}, baseUrl);
};
var resolveFn;
var ata;
var getTypesFromAta = async (code) => new Promise(async (resolve) => {
  if (!code?.trim()) {
    resolve([]);
    return;
  }
  ...
  const ataTypes = [];
  resolveFn = resolve;
  ata = ata || setupTypeAcquisition({
    projectName: "Playground",
    typescript: worker.ts,
    logger: {
      log: () => void 0,
      error: () => void 0,
      groupCollapsed: () => void 0,
      groupEnd: () => void 0
    },
    delegate: {
      receivedFile: (code2, path) => {
        ataTypes.push({ content: code2, filename: path });
      },
  ...
  async (event) => {
    ...
    if (message.type === "ts-features") {
      ...
      if (feature === "getOptionDeclarations") {
        const optionDeclarations = worker.ts?.optionDeclarations.map(
          (x) => objectFilter(x, (value) => {
      ...
      if (feature === "initCodeMirrorTS") {
        if (data && codemirrorWorker.language !== data && typeof codemirrorWorker.changeLanguage === "function") {
...
var initCodemirrorTS = doOnce(async () => {
  await loadTypeScript();
  ...
  const { createDefaultMapFromCDN, createSystem, createVirtualTypeScriptEnvironment } = worker.typescriptVFS;
  ...
  const createTypeScriptEnvironment = (lang) => {
  ...
  codemirrorWorker.changeLanguage = async (lang) => {
