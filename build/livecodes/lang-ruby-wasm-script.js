...
(() => {
  ...
  var rubyWasmBaseUrl = /* @__PURE__ */ getUrl("@ruby/3.4-wasm-wasi@2.7.2/dist/");

  // src/livecodes/languages/ruby-wasm/lang-ruby-wasm-script.ts
  parent.postMessage({ type: "loading", payload: true }, "*");
  livecodes.rubyWasm = livecodes.rubyWasm || { stdlibLoaded: false };
  livecodes.rubyWasm.run = livecodes.rubyWasm.run || (async () => {
    parent.postMessage({ type: "loading", payload: true }, "*");
    ...
    const hasImports = (str) => new RegExp(/require\s+(?:"|')(\S+)(?:"|')/gm).test(str);
    let code = "";
