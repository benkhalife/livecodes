...
`;
  ...
  var runOutsideWorker3 = async (code, { baseUrl, config }) => {
    const { pgSqlCompiler } = await import(baseUrl + "lang-postgresql-compiler-esm.js");
    return pgSqlCompiler(code, { baseUrl, config });
  };

  // src/livecodes/languages/rescript/lang-rescript.ts
  var runOutsideWorker4 = async (code, { baseUrl, language }) => {
    const { rescriptCompiler } = await import(baseUrl + "lang-rescript-compiler-esm.js");
    return rescriptCompiler(code, { baseUrl, language });
  };

  // src/livecodes/compiler/compile.page.ts
  window.compilers = {
    rescript: runOutsideWorker4,
    reason: runOutsideWorker4,
    ocaml: runOutsideWorker4,
    mdx: runOutsideWorker2,
    diagrams: runOutsideWorker,
    postgresql: runOutsideWorker3
  };
})();
