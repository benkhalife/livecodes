...
var require_deep_diff = __commonJS({
  "node_modules/deep-diff/index.js"(exports, module) {
    (function(root, factory) {
      var deepDiff = factory(root);
      if (false) {
        (void 0)("DeepDiff", function() {
          return deepDiff;
        });
      ...
      function arrayRemove(arr, from2, to) {
        var rest = arr.slice((to || from2) + 1 || arr.length);
        arr.length = from2 < 0 ? arr.length + from2 : from2;
      ...
      function applyArrayChange2(arr, index, change) {
        if (change.path && change.path.length) {
          ...
          switch (change.kind) {
            case "A":
              applyArrayChange2(it[change.path[i]], change.index, change.item);
              break;
            case "D":
          ...
          switch (change.kind) {
            case "A":
              applyArrayChange2(arr[index], change.index, change.item);
              break;
            case "D":
      ...
      function applyChange2(target, source, change) {
        if (typeof change === "undefined" && source && ~validKinds.indexOf(source.kind)) {
          change = source;
        }
        if (target && change && change.kind) {
          var it = target, i = -1, last2 = change.path ? change.path.length - 1 : 0;
          while (++i < last2) {
          ...
          switch (change.kind) {
            case "A":
              ...
              applyArrayChange2(change.path ? it[change.path[i]] : it, change.index, change.item);
              break;
            case "D":
              delete it[change.path[i]];
              break;
        ...
        if (target && source) {
          var onChange = function(change) {
            if (!filter || filter(target, source, change)) {
              applyChange2(target, source, change);
            }
          };
...
var getContent = async ({
  ...
  const url = `https://api.github.com/repos/${user.username}/${repo}/contents/${path}` + (branch ? `?ref=${branch}` : "");
...
var createStorage = async (name, isEmbed) => {
  if (isEmbed)
    return fakeStorage;
  let store;
  const pub = createPub();
  ...
  const getItem2 = async (id) => {
    await load();
    return store.getItem(id);
  };
  const updateItem = async (id, value, notify = true) => {
    await load();
...
var createProjectStorage = async (name, isEmbed) => {
  const storage = await createStorage(name, isEmbed);
...
var initializeStores = async (stores3, isEmbed) => {
  if (isEmbed)
    return;
  stores3.projects = await createProjectStorage("__livecodes_data__", isEmbed);
  stores3.templates = await createProjectStorage("__livecodes_templates__", isEmbed);
  stores3.assets = await createStorage("__livecodes_assets__", isEmbed);
  stores3.snippets = await createStorage("__livecodes_snippets__", isEmbed);
  stores3.recover = createSimpleStorage("__livecodes_project_recover__", isEmbed);
  stores3.userConfig = createSimpleStorage("__livecodes_user_config__", isEmbed);
  stores3.userData = await createStorage("__livecodes_user_data__", isEmbed);
  stores3.appData = createSimpleStorage("__livecodes_app_data__", isEmbed);
  stores3.sync = await createStorage("__livecodes_sync_data__", isEmbed);
};
var getStoreKey = (store, stores3) => {
  for (const key of Object.keys(stores3)) {
    if (stores3[key] === store) {
      return key;
    }
  }
  return null;
...
// src/livecodes/sync/diff.ts
var DeepDiff = __toESM(require_deep_diff());
...
function getEntries(source) {
  ...
  if (source instanceof YArray) {
    return source.map((value, index) => [index, value]);
  }
  if (source instanceof YMap) {
    return [...source.entries()];
  }
  return [];
}
function setValue(target, key, value, mapFn = toYjsType) {
  ...
  if (target instanceof YArray && typeof key === "number") {
...
function applyChange(target, change) {
  if (target && change && change.kind) {
    let it = target;
    let i = -1;
    const last2 = change.path ? change.path.length - 1 : 0;
    while (++i < last2) {
      if (typeof it.get(change.path?.[i]) === "undefined") {
        setValue(
          it,
          change.path?.[i],
          typeof change.path?.[i + 1] === "number" ? new YArray() : new YMap()
        );
      }
      it = it.get(change.path?.[i]);
    }
    switch (change.kind) {
      case "A":
        if (change.path && typeof it.get(change.path[i]) === "undefined") {
          setValue(it, change.path[i], new YArray());
        }
...
var storesToSync = ["projects", "templates", "assets", "snippets"];
var repoDir = "livecodes-data";
var rootArrayKey = "data";
var worker = self;
var stores2 = createStores();
var getStorageData = async (storage) => {
  if (!storage)
    return [];
  if ("getValue" in storage) {
    const storeKey = getStoreKey(storage, stores2);
...
var setStorageData = async (storage, data) => {
  if (!storage)
    return;
...
var syncStore = async ({
  ...
  const lastSyncSha = (await stores3.sync?.getItem(syncKey))?.lastSyncSha;
  const filename = `${storeKey}.b64.zip`;
  const path = `${repoDir}/${filename}`;
  const storage = stores3[storeKey];
  if (!storage)
    return true;
  const isSimpleStorage = "getValue" in storage;
  if (isSimpleStorage)
    return true;
  const doc2 = new yjs_exports.Doc();
  try {
    let remoteUpdate;
    ...
    const remoteFile = !remoteFileExists || uptodate ? void 0 : await getContent({
      user,
      repo,
      branch,
      path
    ...
    const localUpdate = (await stores3.sync?.getItem(syncKey))?.data;
    const currentData = await getStorageData(storage);
    if (localUpdate) {
      yjs_exports.applyUpdate(doc2, localUpdate);
      changeDoc(doc2.getArray(rootArrayKey), currentData);
      if (remoteUpdate) {
        yjs_exports.applyUpdate(doc2, remoteUpdate);
      }
    }
    ...
    if (!localUpdate && remoteUpdate) {
      const remoteDoc = new yjs_exports.Doc();
      yjs_exports.applyUpdate(remoteDoc, remoteUpdate);
      const remoteData = toJSON(remoteDoc.getArray(rootArrayKey));
      remoteDoc.destroy();
      const data = [...remoteData, ...currentData];
      changeDoc(doc2.getArray(rootArrayKey), data);
    }
    const newData = toJSON(doc2.getArray(rootArrayKey));
    const dataChanged = DeepDiff.diff(newData, currentData) != null;
    if (dataChanged) {
      await setStorageData(storage, newData);
    }
    const newSyncUpdate = yjs_exports.encodeStateAsUpdate(doc2);
    const shouldPushUpdate = !remoteFileExists || remoteUpdate && !typedArraysAreEqual(remoteUpdate, newSyncUpdate) || uptodate && localUpdate && !typedArraysAreEqual(localUpdate, newSyncUpdate);
    if (shouldPushUpdate) {
      const file = {
        path,
    ...
    const dirEntries = !shouldPushUpdate ? remoteContent : (await getContent({
      user,
      repo,
      branch,
      path: repoDir
    ...
    await stores3.sync?.updateItem(syncKey, newSyncData);
  } catch {
    return false;
  } finally {
    doc2.destroy();
...
var sync = async ({
  ...
  try {
    if (newRepo) {
      ...
      const repoRootEntries = (await getContent({
        user,
        repo,
        branch,
        path: ""
      ...
      if (repoRootEntries?.find?.((x) => x.type === "dir" && x.name === repoDir)) {
        remoteContent = (await getContent({
          user,
          repo,
          branch,
          path: repoDir
...
var exportToLocalSync = async ({ user, storeKey }) => {
  const syncKey = `${user.username}_${storeKey}`;
  const storage = stores2[storeKey];
  if (!storage)
    return;
  const { data: localUpdate, lastSyncSha = "" } = await stores2.sync?.getItem(syncKey) || {};
  const currentData = await getStorageData(storage);
  const doc2 = new yjs_exports.Doc();
  try {
    if (localUpdate) {
    ...
    await stores2.sync?.updateItem(syncKey, newSyncData);
  } finally {
    doc2.destroy();
  }
};
var exportStoreAsBase64Update = async ({ storeKey }) => {
  const storage = stores2[storeKey];
  if (!storage)
    return;
  const currentData = await getStorageData(storage);
  const doc2 = new yjs_exports.Doc();
  changeDoc(doc2.getArray(rootArrayKey), currentData);
  const update = yjs_exports.encodeStateAsUpdate(doc2);
  const base64 = Uint8ArrayToBase64(update);
  doc2.destroy();
  return base64;
};
var restoreFromUpdate = async ({
  update,
  storeKey,
  mergeCurrent = true
}) => {
  ...
  const doc2 = new yjs_exports.Doc();
  try {
    yjs_exports.applyUpdate(doc2, update);
    if (mergeCurrent) {
      const savedData = toJSON(doc2.getArray(rootArrayKey));
...
var restoreFromLocalSync = async ({
  user,
  storeKey,
  mergeCurrent = true
}) => {
  ...
  if (!storage)
    return;
  const update = (await stores2.sync?.getItem(syncKey))?.data;
  if (!update)
    return;
  return restoreFromUpdate({ update, storeKey, mergeCurrent });
};
var createZip = async (files) => {
...
var extractZip = async (content, encoding) => {
  const JSZip = await loadScript(jsZipUrl, "JSZip");
...
worker.addEventListener("message", async (event) => {
  if (!stores2.projects) {
    await initializeStores(stores2, false);
  }
  const message = event.data;
  let data;
  if (message.method === "sync") {
    data = await sync(...message.args);
  }
  if (message.method === "exportToLocalSync") {
    data = await exportToLocalSync(...message.args);
  }
  if (message.method === "exportStoreAsBase64Update") {
    data = await exportStoreAsBase64Update(...message.args);
  }
  if (message.method === "restoreFromUpdate") {
    data = await restoreFromUpdate(...message.args);
  }
  if (message.method === "restoreFromLocalSync") {
    data = await restoreFromLocalSync(...message.args);
  }
  worker.postMessage({
