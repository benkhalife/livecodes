...
(() => {
  ...
  window.addEventListener("load", async () => {
    ...
    async function main() {
      if (livecodes.pyodideLoading === false)
        return;
...
`.trim();
    ...
    async function loadPackagesInCode(code2) {
      const pkgMap = {
        skimage: "scikit-image",
        sklearn: "scikit-learn"
      };
      const packages = [...livecodes.pyodide.pyodide_py.code.find_imports(code2)];
      const newPackages = packages.filter(
        (p) => !(p in livecodes.pyodide.loadedPackages) && !livecodes.excludedPackages.includes(p)
      );
      for (const p of newPackages) {
        const pkg = pkgMap[p] ?? p;
        try {
    ...
    async function evaluatePython(code2) {
      await pyodideReady;
      await cleanUp();
      await prepareEnv();
      await loadPackagesInCode(code2);
      try {
        livecodes.pyodideState = livecodes.pyodide.pyodide_py._state.save_state();
        await livecodes.pyodide.runPythonAsync(code2);
      } catch (err) {
        console.log(err);
      }
    }
