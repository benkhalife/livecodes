...
(() => {
  ...
  // src/livecodes/languages/handlebars/lang-handlebars-compiler.ts
  self.createHandlebarsCompiler = () => async (code, { config }) => {
    ...
    if (config.customSettings.template?.prerender !== false) {
      const template = self.Handlebars.compile(code, options);
      return template(data);
    }
    const templateSpec = self.Handlebars.precompile(code, options);
    return `<!-- ... compiling ... -->
